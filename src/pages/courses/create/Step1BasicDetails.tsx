...
//       return
//     }
//     const existing = document.querySelector('script[src*="tpstreams-uploader"]') as HTMLScriptElement | null
//     if (existing) {
...
const Step1BasicDetails = ({ form, update, setIsDraft, isSubmitting = false, isEdit, courseId }: Props) => {
  ...
  const initSDK = () => {
    ...
    uploader.on('uploadSuccess', async (data: any) => {
      ...
      // try {
      //   // Save asset_id to DB via courseService
      //   await courseService.saveIntroVideo(courseId, data.asset_id)
      //   formik.setFieldValue('intro_video_asset_id', data.asset_id)
      setUploadStatus('done')
      //   toast.success('Intro video uploaded successfully')
      // } catch (err: any) {
      //   console.error('Save intro video error:', err)
      //   setUploadStatus('failed')
  ...
  const handleVideoFile = (file: File) => {
    ...
    // ✅ Correct SDK usage from TPStreams docs
