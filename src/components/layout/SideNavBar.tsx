...
const SideNavBar = () => {
    ...
    // Expanded when hovered (mouse) OR tapped open (touch) OR account menu open.
    const expanded = isHovered || clickOpen || accountOpen
    const collapsed = !expanded

    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            const target = e.target as Node
            // Clicking anywhere outside the sidebar collapses it (and closes the menu).
            if (asideRef.current && !asideRef.current.contains(target)) {
                setClickOpen(false)
                setAccountOpen(false)
            } else if (accountRef.current && !accountRef.current.contains(target)) {
                // Inside the sidebar but outside the account menu → close the menu only.
                setAccountOpen(false)
            }
        }
    ...
    // Collapse the expanded sidebar on scroll — TABLET ONLY. On laptop (lg+) we
    // keep the original hover-only behaviour, so scrolling does nothing there.
    useEffect(() => {
        const collapse = () => {
    ...
    return (
        <div className="shrink-0 h-screen" style={{ width: COLLAPSED_WIDTH }}>
            <aside
                ...
                className={`fixed left-0 top-0 h-screen bg-input-bg border-r border-gray-100 flex flex-col py-6 overflow-hidden z-50 ${expanded ? 'shadow-[4px_0_24px_rgba(0,11,96,0.1)]' : ''
                    }`}
            >
                {/* Logo */}
                <div
